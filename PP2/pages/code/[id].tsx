...
const Code = () => {
 // set the tmeplate
  ...
  // -------------- start of difference with the index.tsx file
  const { id } = router.query; // Get the template ID from the URL

  // render in the template data 
  useEffect(() => {
    if (!id) return; 
    const fetchTemplate = async () => {
      if (!id) return; // If the ID is not available, exit
          ...
          setSelectedLanguage(data.template.language)
          setCode(data.template.content)
  ...
  // -------------- end difference with the index.tsx file
