...
export default async function handler(req, res) {
    ...
    if (req.method === "GET") {
        ...
        try {
            ...
            const totalBlogPages = Math.ceil(totalBlogCount / limit);
            const totalCommentCount = await prisma.comment.count();
            const totalCommentPages = Math.ceil(totalCommentCount / limit);
            
            return res.status(200).json({ sortedBlogs, sortedComments, totalBlogCount, totalBlogPages, totalCommentCount, totalCommentPages, page, limit });
        } catch (error) {
            console.log(error);
